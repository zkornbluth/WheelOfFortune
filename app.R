...
wheel_data <- read_csv("wheeloffortune.csv", 
                       col_types = cols(Date = col_date(format = "%B %d, %Y")))
supplied_letters = c('R', 'S', 'T', 'L', 'N', 'E') # players are given RSTLNE before they choose more

wheel_data$puzzle_letters <- wheel_data$Puzzle %>% 
  str_replace_all("[^[a-zA-Z]]", "")
wheel_data$puzzle_length <- nchar(wheel_data$puzzle_letters)
wheel_data <- wheel_data %>% rowwise() %>% mutate(num_revealed_letters = sum(str_count(puzzle_letters, supplied_letters)))
...
getLetterFreq <- function(startYear, endYear, category=NULL) {
  ...
  for (i in 1:nrow(letter_freq_data)) {
    ...
    contains_letter = str_detect(games_to_check$puzzle_letters, letter)
